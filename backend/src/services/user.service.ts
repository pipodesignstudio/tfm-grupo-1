...
import { BadRequestError, CustomError, InternalServerError, NotFoundError } from "../utils";
...
export class UserService {
  ...
  */
   ...
   */
  public async updateProfile(userId: number, dto: UpdateUserDto): Promise<Omit<IUser, 'contrasena' | 'id'>> {
    const updateData: {
      nombre?: string | null;
    ...
    if (dto.img_perfil !== undefined) {
      if (dto.img_perfil === null) {
        ...
        try {
          ...
          throw new BadRequestError('La imagen de perfil no se ha podido decodificar  ', { error: 'BAD_REQUEST' }); // O manejarlo como un error de validación
        }
      }
    }
    ...
    if (Object.keys(updateData).length === 0) {
        const existingUser = await prisma.usuarios.findUnique({
            where: { id: userId },
            select: {
                email: true, nombre: true, apellido: true, nick: true, img_perfil: true,
                primera_sesion: true, fecha_creacion: true, borrado: true, email_verificado: true
            }
        });
        if (!existingUser) {
